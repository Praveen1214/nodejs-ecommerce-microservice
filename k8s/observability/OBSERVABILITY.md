...
1. *Step 1 — Existing app deploy*
kubectl get pods -n ecommerce-test

2. *Step 2 — Monitoring Namespace*
...
3. *Step 3 — kube-prometheus-stack install (Node Exporter + Kubelet/cAdvisor + Prometheus + Grafana)*
...
4. *Step 4 — Istio install + sidecar injection enable (Mesh metrics + Graph edges)*
...
5. *Step 5 — App-level metrics (/metrics) enable (Prometheus client)*
...
6. *Step 6 — Prometheus ServiceMonitors create (App + Istio metrics scrape)*
...
spec:
  ...
  selector:
    matchLabels:
      ...
      environment: test
  endpoints:
    - port: "3000"     # IMPORTANT: service port name දාන්න ඕන
      path: /metrics
      interval: 15s
---
apiVersion: monitoring.coreos.com/v1
...
7. *Step 7 — Grafana + Prometheus access (verify metrics exist)*
**Prometheus UI**
...
avg by (pod, namespace) (
  rate(container_cpu_usage_seconds_total{namespace="ecommerce-test", container!="", image!=""}[1m])
) * 100

**pod_memory_usage_mb_avg**

avg by (pod, namespace) (
  container_memory_working_set_bytes{namespace="ecommerce-test", container!="", image!=""}
) / 1024 / 1024
